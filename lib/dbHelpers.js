 ...
 * @param {Array<string>} allowedRoles - Array of allowed roles (optional)
 * @returns {Promise<{user: {uid: string, email: string, role?: string} | null, error: string | null}>}
 */
export async function getAuthenticatedUser(req = null, allowedRoles = null) {
  try {
    ...
    // Get role from custom claims first, then fallback to Firestore
    let role = decoded.role || decoded.customClaims?.role;
    
    // If no role in session, fetch from Firestore
    if (!role) {
...
/**
 * Check if user has faculty mentor role
 * @param {Object} user - User object from getAuthenticatedUser
 * @returns {boolean}
 */
export function isFacultyMentor(user) {
  return user?.role === 'faculty-mentor';
}
